  ...
  this._wss.on('connection', function(socket) {
    ...
    if (!self._clients[key] || !self._clients[key][id]) {
      ...
      for (peerid in self._clients[key]) {
            if ((self._clients[key].hasOwnProperty(peerid) && peerid !== id) && self._clients[key][peerid].socket !== undefined) {
      ...
      self._removePeer(key, id);
        for (peerid in self._clients[key]) {
            if (self._clients[key].hasOwnProperty(peerid) && peerid !== id) {
                self._clients[key][peerid].socket.send(JSON.stringify({ type: 'PEERDISCONNECTED', payload: { id: id}}));
            }
        }
    }
  });
...
app._initializeHTTP = function() {
  ...
  this.get('/:key/id', function(req, res, next) {
    ...
    res.send(self._generateClientId(req.params.key, req.connection.remoteAddress));
  });
...
app._generateClientId = function(key, ip) {
    var clientId = util.randomId(ip);
    if (!this._clients[key]) {
    return clientId;
  }
  while (!!this._clients[key][clientId]) {
    clientId = util.randomId(ip);
  }
  return clientId;
};
